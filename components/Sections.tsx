...
import { Download, Github, ArrowUpRight, Cpu, ShieldCheck, Zap, Database, Globe, Layers, Server } from 'lucide-react';
...
export const SectionIngress: React.FC = () => (
  <section className="min-h-screen flex flex-col justify-center px-12 md:px-24 relative">
    ...
    <h1 className="text-8xl md:text-[11rem] font-black tracking-tighter leading-[0.85] mb-12 uppercase italic">
      M. BILAL<br />
      <span className="text-outline text-transparent opacity-40" style={{ WebkitTextStroke: '2px rgba(255,255,255,0.4)' }}>TAHIR</span>
    </h1>
    <p className="max-w-2xl text-white/50 text-xl leading-relaxed font-light mb-16 border-l-2 border-[#2E5BFF] pl-10 italic">
      Full-Stack & AI Engineer. Specialized in <span className="text-white font-bold">Python, MERN, and AI Agentics</span>. 
      Architecting high-performance systems from <span className="text-[#2E5BFF] font-black uppercase">Islamabad, Pakistan</span>.
    </p>
    ...
    <div className="flex gap-10">
      <a 
        ...
        target="_blank" 
        className="bg-[#2E5BFF] px-12 py-6 mono text-[12px] font-black tracking-[0.4em] flex items-center gap-6 hover:shadow-[0_0_30px_#2E5BFF/40] transition-all uppercase italic"
      >
        PULL_RESUME_v2.5 <Download size={18} />
      </a>
    </div>
  </section>
);
...
export const SectionIntelligence: React.FC = () => {
  const stack = [
    { cat: "ARCH_STACK", items: ["Python", "FastAPI", "Node.js", "Express"] },
    { cat: "AI_MATRIX", items: ["LangChain", "DSPy", "LangGraph", "Vector DBs"] },
    { cat: "UI_COMMAND", items: ["React 19", "Next.js 15", "Tailwind", "GSAP"] },
    { cat: "CLOUD_OPS", items: ["Azure Functions", "Vercel", "PostgreSQL", "MongoDB"] }
  ];
  ...
  return (
    <section className="py-32 px-12 md:px-24">
      <div className="mb-24 flex items-center justify-between border-b border-white/5 pb-10">
        ...
        <span className="mono text-[10px] text-white/20 tracking-[0.5em] uppercase">SYSTEM_DNA_02</span>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        {stack.map((s, i) => (
          <GlassBox key={i} className="bg-black/40">
            <h3 className="mono text-[12px] font-black text-[#2E5BFF] mb-8 tracking-[0.3em] uppercase italic">{s.cat}</h3>
...
export const SectionDeployment: React.FC = () => (
  <section className="py-32 px-12 md:px-24">
    <div className="mb-24 border-l-8 border-[#2E5BFF] pl-12">
      ...
      <p className="mono text-[11px] text-[#2E5BFF] font-black mt-2 tracking-[0.4em] uppercase">PROFESSIONAL_DEPLOYMENT_RECORDS</p>
    </div>
    ...
    <div className="relative border-l border-white/10 pl-12 space-y-24">
      <div className="relative group">
        ...
        <ul className="space-y-6 max-w-4xl">
          {[
            "Developed serverless Python backends using FastAPI deployed on Azure Functions for scalable document intelligence.",
            "Engineered Autonomous AI Agent Systems utilizing LangChain and DSPy for multi-step reasoning tasks.",
            "Optimized RAG pipelines and integrated LangGraph for complex, stateful AI workflow automation."
          ].map((log, i) => (
...
export const SectionArchive: React.FC = () => {
  const projs = [
    { 
      title: "Playistan", 
      tags: ["MERN", "Socket.IO", "OTP MFA"], 
      link: "https://github.com/IfBilal/Playistan-ISE", 
      desc: "Lead Full Stack Developer. Real-time sports booking platform with bidirectional community chat and secure OTP-based identity management." 
    },
    { 
      title: "YouGram", 
      tags: ["React", "MongoDB", "Aggregates"], 
      link: "https://github.com/IfBilal/You-Gram", 
      desc: "Enterprise Social Platform. Orchestrated complex MongoDB Aggregation Pipelines to deliver personalized content discovery feeds." 
    }
  ];
  ...
  return (
    <section className="py-32 px-12 md:px-24 bg-black/40">
      <div className="mb-24 flex justify-between items-end border-b border-white/5 pb-10">
        ...
        <div className="mono text-[9px] text-white/10 hidden md:block uppercase tracking-[0.3em]">INTEGRITY: VERIFIED</div>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
        {projs.map((p, i) => (
          <GlassBox key={i} className="min-h-[420px] flex flex-col justify-between border border-white/5">
            <div>
              ...
              <div className="flex gap-4 flex-wrap">
                {p.tags.map(t => <span key={t} className="mono text-[9px] bg-white/5 px-4 py-1 text-white/50 border border-white/10 uppercase font-black">{t}</span>)}
              </div>
            </div>
            <a href={p.link} target="_blank" className="mono text-[11px] font-black tracking-[0.4em] text-[#2E5BFF] flex items-center gap-4 hover:gap-8 transition-all pt-10 border-t border-white/5 uppercase italic">
              ACCESS_SOURCE <ArrowUpRight size={16} />
            </a>
          </GlassBox>
        ))}
      </div>
...
export const SectionUplink: React.FC = () => (
  <section className="py-64 px-12 md:px-24 text-center">
    <div className="inline-block mono text-[11px] text-[#2E5BFF] font-black tracking-[1.2em] mb-12 animate-pulse uppercase italic">ESTABLISHING_SYNC</div>
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-20">
      <div className="space-y-4">
        <p className="mono text-[10px] text-white/20 tracking-[0.5em] uppercase">Relay_Primary</p>
      ...
      <div className="space-y-4">
        <p className="mono text-[10px] text-white/20 tracking-[0.5em] uppercase">Sync_Node</p>
        <div className="flex justify-center gap-10">
          <a href="https://linkedin.com/in/m-bilaltahir" target="_blank" className="text-xl font-bold hover:text-[#2E5BFF] transition-colors italic uppercase">LINKEDIN</a>
          <a href="https://github.com/IfBilal" target="_blank" className="text-xl font-bold hover:text-[#2E5BFF] transition-colors italic uppercase">GITHUB</a>
        </div>
      </div>
      <div className="space-y-4">
        <p className="mono text-[10px] text-white/20 tracking-[0.5em] uppercase">Voice_Encrypted</p>
        <p className="text-xl font-bold italic text-white/80">+92-335-5558223</p>
      </div>
    </div>
  </section>
);
